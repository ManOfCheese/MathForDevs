...
public class Game : MonoBehaviour {
    ...
    private void Start() {
        ...
        for(int i = 0; i < 1; ++i) {
            enemies.Add( new Enemy( new DevMath.Vector2( Random.Range( 0.0f, Screen.width ), Random.Range( 0.0f, Screen.height ) ) ) );
        }
    ...
    private void OnGUI() {
        ...
        if ( player == null ) {
            ...
            GUI.Label( new Rect( Screen.width * 0.5f - 50.0f, Screen.height * 0.5f - 10.0f, 100.0f, 100.0f ), "YOU LOSE!" );
        }
        else {
            if ( Input.GetKey( KeyCode.Q ) ) {
                foreach ( var enemy in enemies ) {
                    ...
                    var angle = DevMath.DevMath.RadToDeg( DevMath.Vector2.Angle( new DevMath.Vector2( 0, 0 ), direction ) );
                    Matrix4x4 pivot = Matrix4x4.Translate( player.Position.ToUnity() );
                    ...
                    var textPos = player.Position + DevMath.Vector2.DirectionFromAngle( angle ) * ( distance * 0.5f );
